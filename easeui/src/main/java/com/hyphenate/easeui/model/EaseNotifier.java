...
public class EaseNotifier {
    ...
    public void cancelNotificatonForeground() {
        if (notificationManager != null)
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        if (message.getChatType() == EMMessage.ChatType.Chat) {
            username = message.getStringAttribute(EaseConstant.NICK_NAME, "");
        } else {
            EMGroup emGroup = EMClient.getInstance().groupManager().getGroup(message.getTo());
            username = emGroup.getGroupName();
        }
        Log.i("tag", "============   send   isF" + isForeground + "  num " + numIncrease + " username " + username);
        try {
...
//            String summaryBody = message.getStringAttribute(EaseConstant.NICK_NAME,"")+msgs[0];
